...
export default function Input({ label, error, hint, className = "", id, ...props }) {
  ...
  return (
    <label className="block" htmlFor={inputId}>
      ...
      <input
        id={inputId}
        className={`w-full rounded-xl border bg-white px-3.5 py-2.5 text-sm text-ink-900 placeholder:text-ink-300 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 ${
